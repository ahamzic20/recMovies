...
export class HomeComponent implements OnInit {
  ...
  async getRecomendations() {
    ...
    if (this.userMovies.length < 5) {
      const selectedMovies: IMovie[] = [];

      // Fetch random movie by genre, ensuring it's not already selected
      this.randomMovieByGenre = await this.moviesService.getRandomMovie(
        this.userId as number
      );
      if (
        !selectedMovies.some(
          (movie) => movie.id === this.randomMovieByGenre?.id
        )
      ) {
      ...
      this.randomMovieByActor = await this.moviesService.getRandomMovie(
        this.userId as number
      );
      if (
        !selectedMovies.some(
          (movie) => movie.id === this.randomMovieByActor?.id
        )
      ) {
      ...
      this.randomMovieByDirector = await this.moviesService.getRandomMovie(
        this.userId as number
      );
      if (
        !selectedMovies.some(
          (movie) => movie.id === this.randomMovieByDirector?.id
        )
      ) {
      ...
      this.randomMovie = await this.moviesService.getRandomMovie(
        this.userId as number
      );
      ...
      // Fetch random movie by genre, ensuring it's not already selected
      this.randomMovieByGenre =
        await this.moviesService.getMovieByFavoriteGenre(this.userId as number);
      if (
        !selectedMovies.some(
          (movie) => movie.id === this.randomMovieByGenre?.id
        )
        ...
        this.randomMovieByGenre =
          await this.moviesService.getMovieByFavoriteGenre(
            this.userId as number
          );
      }

      // Fetch random movie by actor, ensuring it's not already selected
      this.randomMovieByActor =
        await this.moviesService.getMovieByFavoriteActor(this.userId as number);
      if (
        !selectedMovies.some(
          (movie) => movie.id === this.randomMovieByActor?.id
        )
        ...
        this.randomMovieByActor =
          await this.moviesService.getMovieByFavoriteActor(
            this.userId as number
          );
      }

      // Fetch random movie by director, ensuring it's not already selected
      this.randomMovieByDirector =
        await this.moviesService.getMovieByFavoriteDirector(
          this.userId as number
        );
      if (
        !selectedMovies.some(
